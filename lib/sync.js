		...
		for (var v in newDoc.versions) {
	        ...
	        if(version && version.dist && version.dist.tarball){
		        ...
		        if(tarball.endsWith('.tgz')){
		        	version.dist.tarball = TARBALL_DATABASE + encodeURIComponent(id)+'/'+tarball
		        } else {
 ...
 */
function _syncDependencies(remoteRegistry, localRegistry, TARBALL_DATABASE, version, whichDependency = 'dependencies') {
    ...
    if(dependencies.length > 0){
        async.each(dependencies, function (dependency) {
            ...
            syncDoc(remoteRegistry, localRegistry, TARBALL_DATABASE, dependency, dependencyVersionRange, {
 ...
 * @param options
 */
function syncDependencies(remoteRegistry, localRegistry, TARBALL_DATABASE, doc, versionNum, options) {
            ...
            if(doc.versions){
                ...
                if(version){
                    if(options.syncDependencies){	// 同步依赖
                        _syncDependencies(remoteRegistry, localRegistry, TARBALL_DATABASE, version)
                    }
                    if(options.syncDevDependencies){	// 同步开发依赖
                        _syncDependencies(remoteRegistry, localRegistry, TARBALL_DATABASE, version, 'devDependencies')
                    }
                } else {
 ...
 * @return {[type]}                [description]
 */
function syncDoc(remoteRegistry, localRegistry, TARBALL_DATABASE, id, versionRange, options) {
	...
	async.parallel({
		...
		if(error){
			...
			// 2.同步依赖
            syncDependencies(remoteRegistry, localRegistry, TARBALL_DATABASE, results.remote, versionNum, options)
            ...
            if(!cache.hasSyncedTarball(id, versionNum)){
				...
				if(!tarballOriginUrl){
					...
					syncTarball(id, tarballOriginUrl, 'http://yaolin:123456@35.201.153.103:5984/tarball/', function(error, result) {
		            	if(error){
	...
	var _syncTarball = function(error, result) {
		if(error){
			...
			if(!result.result){
                ...
                if(result.existed && _.includes(_.keys(result.result._attachments), tarballName)){	// 附件已存在
