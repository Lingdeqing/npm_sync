...
// 默认同步依赖，不同步开发依赖
syncDoc('https://registry.npmjs.org/', 'http://yaolin:123456@35.201.153.103:5984/registry', 'express', null, {
    syncDependencies: true,
...
function syncTarball(id, tarballOriginUrl, tarballRegistry, callback) {
	...
	var _createDoc = function (doc, callback) {
		if(!doc){
			createOrUpdateDoc(tarballRegistry, id, null, {name:id}, null, function (error, result) {
				if(error){
					callback(error)
				} else {
